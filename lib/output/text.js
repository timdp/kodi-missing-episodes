...
module.exports = function(events, config) {
  var cons = require('./text/console')(config),
      prefix = require('./text/prefix')(config);
  ...
  var output = {
    ...
    warn: cons('warn'),
    info: cons('info'),
    debug: config.options.verbose ? cons('info', 'debug') : _.noop
  };

  var showCount, showIndex;

  events.on('kodi_connecting', function() {
    output.debug('Connecting to Kodi');
  });

  events.on('kodi_disconnecting', function() {
    output.debug('Disconnecting from Kodi');
  });

  events.on('kodi_listing_shows', function() {
    output.debug('Getting show list from Kodi');
  });

  events.on('kodi_listed_shows', function(data) {
    output.debug('Found Kodi shows: %d',
      data.shows.length);
  });

  events.on('kodi_getting_episodes', function(data) {
    output.debug('Getting Kodi episodes for %s (#%d)',
      emph(data.title),
      data.id);
  });

  events.on('kodi_got_episodes', function(data) {
    output.debug('Kodi episode count for %s: %d',
      emph(data.title),
      data.episodes.length);
  });

  events.on('tvdb_finding_show', function(data) {
    output.debug('Finding TheTVDB ID for %s',
      emph(data.title));
  });

  events.on('tvdb_getting_episodes', function(data) {
  ...
  events.on('processed_shows', function(data) {
    output.debug('Processed %d shows',
      data.shows.length);
  });

  events.on('processing_show', function(data) {
  ...
  events.on('processed_show', function(data) {
