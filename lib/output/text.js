...
module.exports = function () {
  ...
  var prefix = require('./text/prefix').call(this)
  ...
  this.events.on('kodi_listed_shows', function (data) {
    output.debug('Found Kodi shows: %d', data.shows.length)
  })

  this.events.on('kodi_getting_episodes', function (data) {
    output.debug(
      'Getting Kodi episodes for %s (#%d)',
      emph(data.title),
      data.id
    )
  })
  ...
  this.events.on('tvdb_finding_show', function (data) {
    output.debug('Finding TheTVDB ID for %s', emph(data.title))
  })

  this.events.on('tvdb_getting_episodes', function (data) {
    output.debug(
      'Getting TheTVDB/%s episodes for %s (#%s)',
      data.language,
      emph(data.title),
      data.id
    )
  ...
  this.events.on('excluding_episodes_before', function (data) {
    output.debug(
      'Excluding episodes of %s older than %s',
      emph(data.title),
  ...
  this.events.on('processing_shows', function (data) {
    ...
    showIndex = 0
    output.debug('Processing %d shows', showCount)
  })

  this.events.on('processed_shows', function (data) {
    output.debug('Processed %d shows', data.shows.length)
  })

  this.events.on('processing_show', function (data) {
    output.debug(
      'Processing show %d of %d: %s',
      ++showIndex,
      showCount,
      emph(data.title)
    )
  ...
  this.events.on('processed_show', function (data) {
    if (data.seasons.length) {
      output.warn('%sMissing seasons for %s:', prefix.fail, emph(data.title))
      data.seasons.forEach(function (item) {
        output.warn(
          '%s%s (episodes: %d, aired: %s)',
          prefix.list,
          emph(item.season > 0 ? 'Season ' + item.season : 'Specials'),
          item.episodeCount,
          item.firstAired != null && !isNaN(item.firstAired)
            ? item.firstAired.getFullYear()
            : 'unknown'
        )
      })
    }
    if (data.episodes.length) {
      output.warn('%sMissing episodes for %s:', prefix.fail, emph(data.title))
    ...
    if (!data.seasons.length && !data.episodes.length) {
      output.info(prefix.pass + 'No missing episodes for %s', emph(data.title))
    }
  })
}
