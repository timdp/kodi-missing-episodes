...
var formatDate = function (date) {
  return (date != null && !isNaN(date)) ? moment(date).format('YYYY-MM-DD')
    : 'unknown'
}
  ...
  this.events.on('processed_show', function (data) {
    if (data.seasons.length) {
      ...
      data.seasons.forEach(function (item) {
        output.warn('%s%s (episodes: %d, aired: %s)',
          prefix.list,
          emph(item.season > 0 ? 'Season ' + item.season : 'Specials'),
          item.episodeCount,
          (item.firstAired != null && !isNaN(item.firstAired)) ? item.firstAired.getFullYear()
            : 'unknown')
      })
    }
