...
var filterEpisodes = function (episodes, includeUnaired, schema) {
  var now = new Date()
  return episodes.filter(function (episode) {
    var hasSeason = episode[schema.season] > 0
    var hasAired = episode.firstAired != null && episode.firstAired < now
    return (
      (!config.options.excludeSpecials || hasSeason) &&
      (includeUnaired || hasAired)
    )
  })
}

var groupBySeason = function (episodes, schema) {
  var result = {}
  episodes.forEach(function (episode) {
    var season = episode[schema.season]
    var number = episode[schema.number]
    result[season] = result[season] || {}
    var res = {}
    for (var key in schema.out) {
      if (schema.out[key] != null) {
        res[key] = episode[schema.out[key]]
      }
    }
...
var tvdbInit = function () {
  ...
  tvdb.init(config.tvdb).then(function () {
...
var tvdbFindShow = function (title) {
  ...
  return tvdb.findShow(title).then(function (show) {
...
var tvdbGetEpisodesForShow = function (show) {
  ...
  return tvdb.getEpisodes(show).then(function (episodes) {
...
var tvdbGetEpisodes = function (title) {
  return tvdbFindShow(title)
    ...
    .then(function (episodes) {
      ...
      return groupBySeason(episodes, SCHEMA_TVDB)
    })
}

var kodiConnect = function () {
  events.emit('kodi_connecting')
  return kodi.connect().then(function () {
...
var kodiDisconnect = function () {
  ...
  return kodi.disconnect().then(function () {
...
var kodiGetEpisodes = function (title, id) {
  ...
  return kodi.getEpisodes(title, id).then(function (episodes) {
    events.emit('kodi_got_episodes', _.assign({ episodes: episodes }, data))
    episodes = filterEpisodes(episodes, true, SCHEMA_KODI)
    return groupBySeason(episodes, SCHEMA_KODI)
  })
}

var removeSeasonsBefore = function (firstSeason, tvdbEps) {
  var seasons = Object.keys(tvdbEps)
    .map(toInt)
...
var removeEpisodesBefore = function (firstSeason, firstEpisode, tvdbEps) {
  var episodes = Object.keys(tvdbEps[firstSeason])
    .map(toInt)
  ...
  return function (tvdbSeasonNum) {
    Object.keys(tvdbEps[tvdbSeasonNum])
      ...
      .filter(function (tvdbEpisodeNum) {
        return kodiEps[tvdbSeasonNum][tvdbEpisodeNum] == null
      })
      .sort(compareNum)
...
var processShows = function (shows) {
  ...
  return Q.all(showsSorted.map(limit(processShow))).then(function () {
...
var listAndProcessShows = function () {
  ...
  return kodi.getShows().then(function (shows) {
...
var run = function () {
  return Q.all([tvdbInit(), kodiConnect()]).then(listAndProcessShows)
}
